...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/03/12 18:02:40 by chahan            #+#    #+#             */
/*   Updated: 2022/03/12 18:20:18 by chahan           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	good_args(char **argv, t_fractol *fractol, int argc)
{
	...
	if (!fractol->mlx)
		print_errors("There was a problem with MiniLibx");
	...
	if (!(fractol->window))
		print_errors("There was a problem opening a new Window");
...
int	main(int argc, char **argv)
{
	...
	if (argc >= 2)
	{
		if (!(ft_strcmp("./fractol", argv[0]) || ft_strcmp("fractol", argv[0])))
			print_errors("Executable must be a \"fractol\" file");
